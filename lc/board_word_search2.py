...
@dataclass
class CharMap:
    children: Dict[str, "CharMap"] = field(default_factory=dict)
    coords_paths: Set[Tuple[Tuple[int, int], ...]] = field(default_factory=set)

    def add(self, word: str, coords_path: Tuple[Tuple[int, int], ...]):
        # print(f"add: ------> {word=}, {coords_path=}")
        if len(word) == 0:
            self.coords_paths.add(coords_path)
            return

        child_char_map = self.children.setdefault(word[0], CharMap())
        child_char_map.add(word[1:], coords_path)

    def find_prefixes(self, prefix: str, suffix: str) -> Tuple[str, Tuple[Tuple[int, int], ...]]:
        # print(f"fp 1----------------------------{prefix=}, {suffix=}, {list(self.children.keys())}")
        if len(suffix) > 0:
            child_char_map = self.children.get(suffix[0])
            if child_char_map:
                yield from child_char_map.find_prefixes(prefix + suffix[0], suffix[1:])

        for coords_path in self.coords_paths:
            # print(f"fp ---------------------------- {prefix=}, {suffix=}, {coords_path=}")
            yield prefix, coords_path
...
class Solution:
    ...
    def map_chars(self, board: List[List[str]]) -> None:
        for y in range(len(board)):
            ...
            for x in range(len(row)):
                c = row[x]
                self.char_map.add(c, ((x, y), ))
                self.char_count.setdefault(c, 0)
                self.char_count[c] += 1

    def word_possible(self, word: str) -> bool:
        char_count = {}
        for c in word:
            char_count.setdefault(c, 0)
            char_count[c] += 1
        possible = all([self.char_count.get(c, 0) >= num for c, num in char_count.items()])
        # print(f"--- {word=}: {possible=}")
        return possible

    def explore(self, w: str, prefix: str, coords_path: Tuple[Tuple[int, int], ...]) -> bool:
        x, y = coords_path[-1]
        # print(f"explore  ----- {x=}, {y=}, {w=}, {prefix=}, {coords_path=}")
        ...
        for c in candidates:
            # not out of bounds
            if 0 <= c["y"] < len(board) and 0 <= c["x"] < len(board[0]):
                # not already visited
                if (c["x"], c["y"]) not in coords_path:
                    # correct path
                    if board[c["y"]][c["x"]] == w[0]:
                        new_prefix = prefix + w[0]
                        new_coords_path = coords_path + ((c["x"], c["y"]),)
                        self.char_map.add(new_prefix, new_coords_path)
                        if self.explore(w[1:], prefix=new_prefix, coords_path=new_coords_path):
                            return True
    ...
    def search_word(self, board: List[List[str]], word: str) -> bool:
        if not self.word_possible(word):
            return False

        for p, s in set([(word[i], word[i+1]) for i in range(len(word) - 1)]):
            possible = False
            for prefix, coords_path in self.char_map.find_prefixes("", p):

                if self.explore(s, p, coords_path):
                    possible = True
                    break

            if not possible:
                # print(f"impossible: {word=}, {(p+s)=}")
                return False

        # print(f"possible: {word=}")

        blah = self.char_map.find_prefixes("", word)
        for prefix, coords_path in blah:
            # print(f"================> {prefix=}, {coords_path=}")
            if self.explore(word.removeprefix(prefix), prefix, coords_path):
                return True
